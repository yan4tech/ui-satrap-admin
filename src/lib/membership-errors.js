...
const PAYMENT_MESSAGES = {
  'payment service unavailable':
    'سرویس پرداخت در دسترس نیست. پنجره payment را اجرا کنید (پورت ۲۰۲۲) یا اسکریپت up.ps1 را بزنید.',
  'payment_address is not configured on membership service':
    'سرویس membership به payment متصل نیست (PAYMENT_ADDRESS تنظیم نشده).',
  'branch header does not match user.branch_id':
    'شناسه شعبه با کاربر فعلی هم‌خوانی ندارد. یک‌بار خارج شوید و دوباره وارد شوید.',
  'user header is required': 'نشست کاربر برای API پرداخت معتبر نیست. دوباره وارد شوید.',
};
...
export function extractMembershipErrorMessage(error, fallback = 'خطا در انجام عملیات') {
  ...
  for (const [key, fa] of Object.entries(PAYMENT_MESSAGES)) {
    if (lower.includes(key)) return fa;
  }
